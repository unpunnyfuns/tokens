...
// For specific linting error tests, create minimal fixtures focused on rule testing
// These are not design token spec violations but linting rule violations
const emptySetManifest: UPFTResolverManifest = {
  sets: [{ files: [] }],
  modifiers: {},
};
...
describe("Manifest Linting", () => {
  describe("rule: no-empty-sets", () => {
    it("should error on empty sets", () => {
      const result = lintManifest(emptySetManifest);
