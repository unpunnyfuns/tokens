...
import { writeBundles } from "../src/bundler/index.js";
import { TokenFileReader } from "../src/io/file-reader.js";
import { TokenFileWriter } from "../src/io/file-writer.js";
import { validateTokens } from "../src/validation/index.js";

const BUNDLER_FIXTURES = join(
  process.cwd(),
  "../examples/src/bundler-fixtures",
);
...
async function generateBundlerOutputs() {
  ...
  const fileReader = new TokenFileReader({ basePath: BUNDLER_FIXTURES });
  const fileWriter = new TokenFileWriter();

  // Generate all outputs using the bundler API directly
  for (const { file, desc } of manifests) {
    try {
      // Read the manifest file
      const manifestPath = join(BUNDLER_FIXTURES, file);
      const manifestContent = readFileSync(manifestPath, "utf-8");
      const manifest = JSON.parse(manifestContent);

      // Use the bundler API to generate outputs
      const results = await writeBundles(manifest, {
        fileReader,
        fileWriter,
        basePath: BUNDLER_FIXTURES,
      });
      ...
      if (results.every((r) => r.success)) {
        ...
        console.log(
          `  ${colors.red}✗${colors.reset} Failed: ${desc} (${file})`,
        );
      }
    } catch (error) {
      console.log(
        `  ${colors.red}✗${colors.reset} Failed: ${desc} (${file}) - ${error}`,
      );
    }
  }
}
