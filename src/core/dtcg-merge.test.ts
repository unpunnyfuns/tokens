...
import { dtcgMerge } from "./dtcg-merge.js";
...
describe("dtcgMerge", () => {
  ...
  describe("$type inheritance", () => {
    ...
    it("should error on conflicting $type at group level", () => {
      const a = {
        colors: {
          $type: "color",
          primary: { $value: "#ff0000" },
      ...
      expect(() => dtcgMerge(a, b)).toThrow(/Type conflict.*color.*gradient/);
    });

    it("should error on conflicting token-level $type", () => {
      const a = {
        theme: {
      ...
      expect(() => dtcgMerge(a, b)).toThrow(/Type conflict.*color.*dimension/);
    });
  });

  describe("group vs token conflicts", () => {
    it("should error when merging group into token", () => {
      const a = {
      ...
      expect(() => dtcgMerge(a, b)).toThrow(/Cannot merge group into token/);
    });

    it("should error when merging token into group", () => {
      const a = {
        spacing: {
          small: { $value: "4px" },
          large: { $value: "16px" },
      ...
      expect(() => dtcgMerge(a, b)).toThrow(/Cannot merge token into group/);
    });
  });
  ...
  describe("error handling", () => {
    it("should provide clear error messages with paths", () => {
      ...
      expect(() => dtcgMerge(a, b)).toThrow(/deeply\.nested\.color/);
    });
  });
  ...
  describe("multiple file merging", () => {
    it("should merge multiple token files in order", () => {
      ...
      const result = files.reduce((acc, file) => dtcgMerge(acc, file), {});
  ...
  describe("with example files", () => {
    ...
    it("should merge primitive and semantic tokens", async () => {
      ...
      const merged = dtcgMerge(primitives.tokens, semantic.tokens);
    ...
    it("should merge base and theme tokens", async () => {
      ...
      const merged = dtcgMerge(base.tokens, light.tokens);
    ...
    it("should handle density variants", async () => {
      ...
      const mergedComfortable = dtcgMerge(base.tokens, comfortable.tokens);
      const mergedCompact = dtcgMerge(base.tokens, compact.tokens);
